...
1. Undo boundary — LOAD_SPEC clears history
2. Consistent Enter — all dims blur without advancing
3. Sequential cabinet IDs — B4/W6 not B5134
4. Delete confirmation — modal dialog
5. Delete key — keyboard shortcut with confirmation
6. Gap clickable — click gaps to edit label/width
7. Dimension input styling — all three fields match
8. Type auto-updates face — drawer bank = all drawers
9. Standard width snapping — nearest factory size
10. Wall run + filler — enter wall length, see remaining filler
11. Section picker — + Section shows door/drawer/false front/glass/open

### Remaining Iterations (12-15)

**Discovered through real usage, not pre-planned.** Use the tool as a cabinet maker in Chrome MCP, find the next biggest friction point, fix it.
